...
if __name__ == '__main__':
    ...
    neuron_number = [256]
    layer_number = [4]
    # dropout_values = np.arange(0,10)/10
    dropout_values = [0.0]
    ...
    # RESULTS MATRIX
    mse_mat = np.zeros((len(neuron_number), len(dropout_values)))
    r2_mat = np.zeros((len(neuron_number), len(dropout_values)))

    for i in range(len(neuron_number)):
        config['arch']['neurons'] = neuron_number[i]
        config['arch']['nlayers'] = layer_number[i]
        for j in range(len(dropout_values)):
            config['arch']['drop'] = dropout_values[j]
            ...
            if apartat3 == True:
                aheadpred = config['data']['aheadpred']
                test_x = test_x[0:-aheadpred]
                ypr = np.zeros((len(test_y) - aheadpred, aheadpred))
                testymod = np.zeros((len(test_y) - aheadpred, aheadpred))
                mse = np.zeros((aheadpred, 1))
                for j in range(aheadpred):
                    ypr[:, j] = model.predict(test_x, batch_size=config['training']['batch'], verbose=0)[:, 0]
                    test_x[:, 0:-1] = test_x[:, 1:]
                    test_x[:, -1, -1] = ypr[:, j]
                    testymod[:, j] = test_y[j:-aheadpred + j, -1]
                    mse[j, 0] = mean_squared_error(ypr[:, j], testymod[:, j])
                    print()
            model = 'multistep12'
            np.save('results/mse_' + model + '.npy', mse)
