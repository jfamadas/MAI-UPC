...
# Dropout
dropout_probability = 0.7  # Dropout in the Fully Connected layers // 1.0 means NO dropout
dropout_probability_q = 0.9  # Dropout in the Convolutional layers // 1.0 means NO dropout
...
# CONVOLUTIONAL + POOLING 4
W_conv4 = W_init([5, 5, 128, 256], name='W_conv4')
b_conv4 = b_init([256], name='b_conv4')

h_conv4 = tf.nn.relu(conv2d(h_pool3, W_conv4, name='conv2d') + b_conv4, name='h_conv4')  # shape [batch, 4, 4, 256]
h_conv4_drop = tf.nn.dropout(h_conv4, dropout_q)
h_pool4 = max_pool_2x2(h_conv4_drop, name='h_pool4')  # shape [batch, 2, 2, 256]

# FULLY CONNECTED 1
W_fc1 = W_init([int(np.prod(h_pool4.shape[1:])), 1024], name='W_fc1')
b_fc1 = b_init([1024], name='b_fc1')

# We have to reshape the output volume so it fits the FC layer
h_pool4_reshape = tf.reshape(h_pool4, [-1, int(np.prod(h_pool4.shape[1:]))], name='h_pool4_reshape')
h_fc1 = tf.nn.relu(tf.matmul(h_pool4_reshape, W_fc1) + b_fc1, name='h_fc1')
...
# FULLY CONNECTED 3
W_fc3 = W_init([512, 256], name='W_fc3')
b_fc3 = b_init([256], name='b_fc3')

h_fc3 = tf.nn.relu(tf.matmul(h_fc2_drop, W_fc3) + b_fc3, name='h_fc3')
h_fc3_drop = tf.nn.dropout(h_fc3, dropout_p)

# OUTPUT LAYER
W_fc4 = W_init([256, 10], name='W_fc4')
b_fc4 = b_init([10], name='b_fc4')

labels_predict = tf.add(tf.matmul(h_fc3_drop, W_fc4), b_fc4, name='labels_predict')
...
# Cost function
J1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_real, logits=labels_predict), name='J')
if l2_regularization:
    regul = tf.nn.l2_loss(W_conv1) + tf.nn.l2_loss(W_conv2) + tf.nn.l2_loss(W_conv3) + tf.nn.l2_loss(W_conv4) + \
            tf.nn.l2_loss(W_fc1) + tf.nn.l2_loss(W_fc2) + tf.nn.l2_loss(W_fc3) + tf.nn.l2_loss(W_fc4)
    J = tf.reduce_mean(J1 + beta * regul)
else:
    J = J1
...
for epoch in range(num_epochs):
    ...
    for i in range(steps):
        J_train[epoch] = J_train[epoch] + J1.eval(feed_dict={x: data_train[max_size * i:max_size * (i + 1), :, :, :],
    ...
    J_validation[epoch] = J1.eval(feed_dict={x: data_validation, y_real: labels_validation, dropout_p: 1.0, dropout_q: 1.0})
