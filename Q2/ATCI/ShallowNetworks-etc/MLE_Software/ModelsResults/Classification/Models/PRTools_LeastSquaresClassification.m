...
function Output = PRTools_LeastSquaresClassification (traintest,DataSet,Model,Param);
...
if ~strcmp(traintest,'train') && ~strcmp(traintest,'test')
  error('PRTools_LeastSquaresClassification: String "traintest" invalid');
end;
...
if strcmp(traintest,'train')   %%% Construct the model
  prwarning(2);
  Output = fisherc(DataSet_PRTools);
  prwarning off;
else                           %%% Test the model
