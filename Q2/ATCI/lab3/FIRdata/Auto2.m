...
if isfield(dades,'Classes')
	% si existeix carregar la configuració
    ...
    % llegir les dades d'entrenament
    if exist(dades.FitxerEntradaEnt,'file')
        vars = strsplit([dades.VariablesEntrada, ',' dades.VariableSortida],',')'; 
    ...
    % Llegir el fitxer de codificacio de les variables
    if (dades.FitxerCodif == 1)
		% Llegir el fitxer de codificació
	...
	% Codificar les dades d'entrenament
    nvar = size(vars,1);
    NVar = nvar;
    Algoritme = 'EFP'; % Paràmetre per defecte
	Parametres = 0;
	...
	for i=1:nvar
		var = char(vars(i));
		dat = DadesE.(var);
		% Eliminar els valors missing per buscar els llindars de les classes
		if (miss_data ~= 0)
		...
		%Dat.(var).dat = dades;
		%Dat.(var).Algoritme = 10;
		%Dat.(var).Classes = num2str(Clusters);
		%Dat.(var).From = from;
		%Dat.(var).To = to;
		%Dat.(var).Parametres = [];
		%Dat.(var).c = c;
	...
	% Guardar les dades cofificades al fitxer: 'FDadesCodifi.mat'

	% Cerca de la màscara òptima  
	...
	% Guardar les dades del model
	% Fitxer: 'FDadesModel.mat'
	...
	% Guardar les dades de test
	% Fitxer: 'FDadesEntPred.mat
	...
	if (ErrorInput(1) > 1)
		% Hi ha entrades que no son a la matriu de comportament
		fid = fopen('ERRORF.DAT');
		F = fread(fid,1000);
		str = char(F');
		%msgbox(str,'Forecast Error','warn','modal');
	...
	if (ErrorInput(2) > 0)
		% S'han trobat valors missings
		%msgbox('Forecast warning', 'Missing values','warn');
		uiwait(msgbox('Missing values','Forecast warning','modal'));
    end	
	
	% Guardar els resultats de la predicció
	% Fitxer: 'FDadesPrediccio.mat'
	...
	% Guardar el fitxer de regenerar
	% Fitxer: 'FDadesSortida.mat'
