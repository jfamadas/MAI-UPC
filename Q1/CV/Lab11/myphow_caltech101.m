...
labels_true = zeros(size(imageEstClass));
for i = 1:conf.numClasses
    
   labels_true(1,(conf.numTest+conf.numTrain)*(i-1)+1 : (conf.numTest+conf.numTrain)*i) = i;
    
end

% Plot 8 random images with red or green border
figure(2);
for i = 1:8
    num = floor(size(labels_true,2)*rand())+1;
    im = imread(fullfile(conf.calDir, images{num}));
    
    g = ones(size(im,1)+20,size(im,2)+20);
    if labels_true(num)==imageEstClass(num)
        im2 = cat(3,0*g,255*g,0*g);
    else
        im2 = cat(3,255*g,0*g,0*g);
    end
    im2 = uint8(im2);
    im2(11:end-10,11:end-10,:) = im;
    subplot(4,2,i)
    imshow(im2)
    title(['Predicted class ', classes{imageEstClass(num)}])
...
for i = 1:conf.numClasses
   ...
   if TP == 0
    ...
    fscore = fscore + 2*(pre*recall)/(pre+recall);
   end
end

fscore = fscore/conf.numClasses;
disp(fscore);
